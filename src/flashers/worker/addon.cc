...
public:
  ...
  void HandleOKCallback() {
    ...
    Local<Value> argv[] = {New<v8::String>(msg).ToLocalChecked(),
...
NAN_METHOD(getProgress) {
  ...
  if (TransferWorker::transferer) {
    double sent_size =
        (double)transferer_get_sent_size(TransferWorker::transferer);
    double total_size =
        (double)transferer_get_total_size(TransferWorker::transferer);
