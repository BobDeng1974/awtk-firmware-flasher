...
stream_t *stream_serial_create(const char *port, uint32_t baudrate,
                               serial::bytesize_t bytesize,
                               serial::parity_t parity,
                               serial::stopbits_t stopbits,
                               serial::flowcontrol_t flowcontrol) {
  size_t timeo = 500;
  stream_serial_t *stream =
      (stream_serial_t *)calloc(1, sizeof(stream_serial_t));
  try {
    stream->dev = new serial::Serial(std::string(port), baudrate,
                                     serial::Timeout::simpleTimeout(timeo),
                                     bytesize, parity, stopbits, flowcontrol);
    printf("device=%s baudrate=%d bytesize=%d parity=%d stopbits=%d "
           "flowcontrol=%d\n",
           port, (int)baudrate, (int)bytesize, (int)parity, (int)stopbits,
           (int)flowcontrol);
  } catch (...) {
    free(stream);
    return NULL;
