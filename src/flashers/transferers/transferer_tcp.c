...
static void transferer_tcp_on_progress(void *ctx, const char *tag,
...
static bool_t transferer_tcp_start(transferer_t *t, const char **err_msg) {
  ...
  bool_t ret =
      fymodem_send(tcp->stream, (uint8_t *)buff, size, url->filename, err_msg);
  free(buff);
...
static bool_t transferer_is_valid_url(url_t *url) {
  return url != NULL &&
         strcmp(s_transferer_tcp_creator_desc.name, url->schema) == 0 &&
...
transferer_t *transferer_tcp_create(url_t *url) {
  ...
  transferer_tcp_t *tcp =
      (transferer_tcp_t *)calloc(1, sizeof(transferer_tcp_t));
  tcp->url = url_ref(url);
...
static const char *transferer_tcp_get_props_desc() { return s_props_desc; }
