...
static void transferer_tftp_server_on_progress(void *ctx, const char *tag,
                                               size_t finish, size_t total) {
  (void)tag;
...
static bool_t transferer_tftp_server_start(transferer_t *t,
                                           const char **err_msg) {
...
static bool_t transferer_is_valid_url(url_t *url) {
  return url != NULL &&
         strcmp(s_transferer_tftp_server_creator_desc.name, url->schema) == 0 &&
         url->path;
}
...
transferer_t *transferer_tftp_server_create(url_t *url) {
  ...
  transferer_tftp_server_t *tftp_server =
      (transferer_tftp_server_t *)calloc(1, sizeof(transferer_tftp_server_t));
...
bool_t transferer_tftp_server_register_creator_desc() {
  ...
  s_transferer_tftp_server_creator_desc.get_props_desc =
