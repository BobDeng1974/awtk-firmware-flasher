...
static void on_progress(void *ctx, const char *tag, size_t finish,
                        size_t total) {
  (void)tag;
...
static bool_t transferer_serial_start(transferer_t *t, const char **err_msg) {
  ...
  bytesize_t bytesize =
      s_bytesize ? (bytesize_t)atoi(s_bytesize) : serial::eightbits;
  stopbits_t stopbits =
      s_stopbits ? (stopbits_t)atoi(s_stopbits) : serial::stopbits_one;
  flowcontrol_t flowcontrol = s_flowcontrol ? (flowcontrol_t)atoi(s_flowcontrol)
                                            : serial::flowcontrol_none;

  serial->stream = stream_serial_create(device, baudrate, bytesize, parity,
  ...
  bool_t ret = fymodem_send(serial->stream, (uint8_t *)buff, size,
                            url->filename, err_msg);
  free(buff);
...
static bool_t transferer_is_valid_url(url_t *url) {
  return url != NULL &&
         strcmp(s_transferer_serial_creator_desc.name, url->schema) == 0 &&
         url->host != NULL;
}
...
transferer_t *transferer_serial_create(url_t *url) {
  ...
  transferer_serial_t *serial =
      (transferer_serial_t *)calloc(1, sizeof(transferer_serial_t));
...
static const char *transferer_serial_get_props_desc() {
  ...
  str =
      "[{\"type\":\"options\", \"name\":\"serial port\", \"path\":\"device\",";
  if (len > 0) {
  ...
  // baudrate
  str +=
      ",{\"type\":\"options\", \"name\":\"baudrate\", \"path\":\"baudrate\",";
  str += "\"defValue\":\"115200\", \"options\" : [\"9600\", \"14400\", "
         "\"19200\", \"28800\", \"38400\", \"56000\", \"57600\", \"115200\", "
         "\"230400\", \"460800\", \"921600\"]}";

  // bytesize
  str +=
      ",{\"type\":\"options\", \"name\":\"bytesize\", \"path\":\"bytesize\",";
  str += "\"defValue\":\"8\", \"options\" : [\"5\", \"6\", \"7\", \"8\"]}";

  // stopbits
  str +=
      ",{\"type\":\"options\", \"name\":\"stopbits\", \"path\":\"stopbits\",";
  str += "\"defValue\":\"1\", \"options\" : [\"1\", \"2\"]}";

  // flowcontrol
  str += ",{\"type\":\"options\", \"name\":\"flowcontrol\", "
         "\"path\":\"flowcontrol\",";
  str += "\"defValue\":0, \"options\" : [{\"text\":\"none\", \"value\":0}, "
         "{\"text\":\"software\", \"value\":1}, {\"text\":\"hardware\", "
         "\"value\":2}]}";

  // parity
  str += ",{\"type\":\"options\", \"name\":\"parity\", \"path\":\"parity\",";
  str += "\"defValue\":0, \"options\" : [{\"text\":\"none\", \"value\":0}, "
         "{\"text\":\"odd\", \"value\":1}, {\"text\":\"even\", \"value\":2}]}";
  str += "]";
...
bool_t transferer_serial_register_creator_desc() {
  ...
  s_transferer_serial_creator_desc.get_props_desc =
      transferer_serial_get_props_desc;
