...
void do_put(char *filename) {
  ...
  sprintf(snd_packet.filename, "%s%c%s%c%d%c", filename, 0, "octet", 0,
          blocksize, 0);
  sendto(sock, &snd_packet, sizeof(struct tftpx_packet), 0,
         (struct sockaddr *)&server, addr_len);
  ...
  do {
    ...
    for (rxmt = 0; rxmt < PKT_MAX_RXMT; rxmt++) {
      sendto(sock, &snd_packet, s_size + 4, 0, (struct sockaddr *)&sender,
             addr_len);
      printf("Send %d\n", block);
      // Wait for ACK.
      for (time_wait_ack = 0; time_wait_ack < PKT_RCV_TIMEOUT;
...
void do_list(int sock, char *dir) {
  ...
  sendto(sock, &packet, sizeof(struct tftpx_packet), 0,
         (struct sockaddr *)&server, addr_len);
  ...
  do {
    for (time_wait_data = 0; time_wait_data < PKT_RCV_TIMEOUT * PKT_MAX_RXMT;
      ...
      if (r_size >= 4 && packet.cmd == htons(CMD_DATA) &&
          packet.block == htons(block)) {
        block++;
        ack.block = packet.block;
        sendto(sock, &ack, sizeof(struct tftpx_packet), 0,
               (struct sockaddr *)&sender, addr_len);
